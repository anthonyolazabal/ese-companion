  ...
  methods: {
    async submit() {
      if (this.item.permission_string) {
        await this.$axios
          .put('/api/rest_api_permission/' + this.$route.query.id, this.item)
          .then((response) => {
            createToast("Rest Api permission updated successfully", {
              type: 'success',
              position: 'bottom-center',
            })
          }).then(() => router.push('/rest-api-permissions')).catch((error) => {
  ...
  <VRow>
    <VCol cols="12" md="12">
      <v-breadcrumbs>
        ...
        <v-breadcrumbs-item>
          <RouterLink :to="'/rest-api-permissions-edit?id' + $route.query.id">
            Edit Permission
          </RouterLink>
        </v-breadcrumbs-item>
    ...
    <VCol cols="12">
      <VCard title="Update a rest api permission">
        <VCardItem>
          <VForm>
