...
export default {
  ...
  methods: {
    ...
    deleteItem(val: any) {
      router.push('/cc-users-delete?id=' + val.id)
    },

    viewItem: function (val: any) {
      router.push('/cc-users-details?id=' + val.id)
    },

    editItem: function (val: any) {
      router.push('/cc-users-edit?id=' + val.id)
    }
  },
  async mounted() {
    await this.$axios
...
<template>
  <VRow>
    ...
    <VCol cols="12">
      <VCard title="Control Center Users">
        <VCardItem>
          <VRow>
            <VCol cols="12" md="6">
              List of all control center users declared in the ESE database.
            </VCol>
            <VCol cols="12" md="6" class="text-right">
              <v-btn variant="outlined" size="small" to="/cc-users-add">
                New Control center user
              </v-btn>
            </VCol>
          </VRow>
        </VCardItem>
