...
export default {
  ...
  methods: {
    ...
    deleteItem(val: any) {
      router.push('/mqtt-accounts-delete?id=' + val.id)
    },

    viewItem: function (val: any) {
      router.push('/mqtt-accounts-details?id=' + val.id)
    },

    editItem: function (val: any) {
      router.push('/mqtt-accounts-edit?id=' + val.id)
    }
  },
  async mounted() {
    await this.$axios
    ...
    <VCol cols="12">
      <VCard title="Accounts">
        <VCardItem>
          <VRow>
            <VCol cols="12" md="6">
              List of all MQTT Accounts declared in the ESE database.
            </VCol>
            <VCol cols="12" md="6" class="text-right">
              <v-btn variant="outlined" size="small" to="/mqtt-accounts-add">
                New Account
              </v-btn>
            </VCol>
          </VRow>
