...
export default {
  ...
  methods: {
    ...
    deleteItem(val: any) {
      router.push('/cc-roles-delete?id=' + val.id)
    },

    viewItem: function (val: any) {
      router.push('/cc-roles-details?id=' + val.id)
    },
  },
  async mounted() {
    await this.$axios
...
<template>
  <VRow>
    ...
    <VCol cols="12">
      <VCard title="Control Center Roles">
        <VCardItem>
          <VRow>
            <VCol cols="12" md="6">
              List of all control center roles declared in the ESE database.
            </VCol>
            <VCol cols="12" md="6" class="text-right">
              <v-btn variant="outlined" size="small" to="/cc-roles-add">
                New Control center role
              </v-btn>
            </VCol>
          </VRow>
        </VCardItem>
