  ...
  methods: {
    async update() {
      if (this.item.name && this.item.description) {
        await this.$axios
          .put('/api/rest_api_role/' + this.$route.query.id, this.item)
          .then((response) => {
            createToast("Rest Api role successfully updated", {
              type: 'success',
              position: 'bottom-center',
            })
            this.item = {} as Rest_Api_Role
            if (this.$route.query.return == "details") {
              router.push('/rest-api-roles-details?id=' + this.$route.query.id);
            } else {
  ...
  <VRow>
    <VCol cols="12" md="12">
      <v-breadcrumbs>
        ...
        <v-breadcrumbs-item>
          <RouterLink :to="'/rest-api-roles-edit?id' + $route.query.id">
            Edit Role
          </RouterLink>
        </v-breadcrumbs-item>
    ...
    <VCol cols="12">
      <VCard title="Edit a rest api role">
        <VCardItem>
          <VForm>
