...
export default {
  ...
  data() {
    return {
      ...
      addRole: false,
      deleteRole: false,
      selectedRole: { name: "" } as CC_Role,
      selectedRoleToDelete: {} as CC_Role,
      permissions: [],
      addPermission: false,
  ...
  methods: {
    ...
    async update() {
      if (this.item.username && this.item.password) {
        await this.$axios
          .put('/api/cc_user/' + this.$route.query.id, this.item)
          .then(() => {
            createToast("Control center user successfully updated", {
              type: 'success',
              position: 'bottom-center',
            })
          })
          .then(() => {
            this.item = {} as CC_User
            router.push('/cc-users')
          })
          .catch(() => {
            createToast("Error updating control center user, contact your admin ! ", {
              type: 'warning',
              position: 'bottom-center',
            })
          });
    ...
    edit() {
      router.push('/cc-users-edit?id=' + this.$route.query.id + "&return=details");
    },

    deleteUser() {
      router.push('/cc-users-delete?id=' + this.$route.query.id);
    },
    ...
    async assignRole() {
      if (this.selectedRole.id != null) {
        ...
        await this.$axios
          .post('/api/cc_user_role', requestData)
          .then((response) => {
            createToast("Control center role assigned successfully", {
              type: 'success',
              position: 'bottom-center',
            })
            this.addRole = false
          ...
          .then(async () => {
            this.selectedRole = { name: "" } as CC_Role
            await this.$axios
              .get('/api/cc_user/' + this.$route.query.id + '?extend=true')
              .then((response) => {
    ...
    async assignPermission() {
      if (this.selectedPermission.id != null) {
        ...
        await this.$axios
          .post('/api/cc_user_permission', requestData)
          .then((response) => {
            createToast("Control center permission assigned successfully", {
              type: 'success',
              position: 'bottom-center',
            })
    ...
    async deleteRoleAssignment() {
      await this.$axios
        .delete('/api/cc_user_role/' + this.$route.query.id + "/" + this.selectedRoleToDelete.id)
        .then(() => {
          createToast("Control center role unassigned successfully", {
            type: 'success',
            position: 'bottom-center',
          })
        ...
        .then(async () => {
          this.selectedRoleToDelete = { name: "" } as CC_Role
          await this.$axios
            .get('/api/cc_user/' + this.$route.query.id + '?extend=true')
            .then((response) => {
              this.item = response.data[0];
        ...
        .catch(() => {
          createToast("Error unassigning control center role to user, contact your admin ! ", {
            type: 'warning',
            position: 'bottom-center',
          })
        });
    ...
    async deletePermissionAssignment() {
      await this.$axios
        .delete('/api/cc_user_permission/' + this.$route.query.id + "/" + this.selectedPermissionToDelete.id)
        .then(() => {
          createToast("Control center permission unassigned successfully", {
            type: 'success',
            position: 'bottom-center',
          })
        ...
        .catch(() => {
          createToast("Error unassigning control center permission to user, contact your admin ! ", {
            type: 'warning',
            position: 'bottom-center',
          })
        });
  ...
  <VRow>
    <VCol cols="12" md="12">
      <v-breadcrumbs>
        ...
        <v-breadcrumbs-item>
          <RouterLink :to="'/cc-users-details?id' + $route.query.id">
            User Details
          </RouterLink>
        </v-breadcrumbs-item>
    ...
    <VCol cols="4">
      <VCard title="Control Center user details">
    ...
    <VCol cols="8">
      ...
      <VWindow v-model="currentTab" class="mt-5">
        <VWindowItem value="Roles">
          <VCard>
            <VRow v-for="role in item['cc_user_roles']">
              <VCol cols="12" md="12">
                <VCardItem>
                  <v-toolbar>
                    <VCardSubtitle style="margin-left: 10px;">
                      <b>{{ role.cc_roles.name.toUpperCase() }}</b> ({{ role.cc_roles.description }})
                    </VCardSubtitle>
                    <VDivider />
                    <v-btn icon color="#F05635">
                      <v-icon @click="deleteRoleWarning(role.cc_roles)">mdi-delete</v-icon>
                    </v-btn>
                  </v-toolbar>
                    ...
                    <v-card>
                      ...
                      <v-card-actions>
                        ...
                        <v-btn color="secondary"
                          @click="deleteRole = false; selectedRoleToDelete = {} as CC_Role">Cancel</v-btn>
                      </v-card-actions>
                    </v-card>
                  </v-dialog>
                </VCardItem>
                ...
                <VCardText v-for="permission in role.cc_roles.cc_role_permissions">
                  <VRow>
                    <VDivider />
                    <VCol cols="12" md="4">
                      {{ permission.cc_permissions.permission_string }}
                    </VCol>

                    <VCol cols="12" md="8">
                      {{ permission.cc_permissions.description }}
                    </VCol>
                  </VRow>
                </VCardText>
              </VCol>
              ...
              <VCol>
                <VCardItem>
                  <v-btn variant="outlined" size="small" @click="loadRoles()">
                    Assign new control center role
                  </v-btn>
        ...
        <VWindowItem value="Permissions">
          ...
          <VCard>
            <VRow v-for="permission in item['cc_user_permissions']">
              <VCol cols="12" md="12">
                <VCardItem>
                  <v-toolbar>
                    <VCardSubtitle style="margin-left: 10px;">
                      {{ permission.cc_permissions.permission_string.toUpperCase() }}
                    </VCardSubtitle>

                    <VDivider />
                    <v-btn icon color="#F05635">
                      <v-icon @click="deletePermissionWarning(permission.cc_permissions)">mdi-delete</v-icon>
                    </v-btn>
                  </v-toolbar>
                  <VCardText>
                    {{ permission.cc_permissions.description }}
            ...
            selectedPermissionToDelete.permission_string
                      ...
                      <v-card-actions>
                        ...
                        <v-btn color="secondary"
                          @click="deletePermission = false; selectedPermissionToDelete = {} as CC_Permission">
                          Cancel</v-btn>
                      </v-card-actions>
                    </v-card>
                  </v-dialog>
              ...
              <VCol>
                <VCardItem>
                  <v-btn variant="outlined" size="small" @click="loadPermissions()">
                    Assign new control center permission
                  </v-btn>
