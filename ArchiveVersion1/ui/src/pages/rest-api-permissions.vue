...
export default {
  ...
  methods: {
    ...
    deleteItem(val: any) {
      router.push('/rest-api-permissions-delete?id=' + val.id)
    },

    editItem: function (val: any) {
      router.push('/rest-api-permissions-edit?id=' + val.id)
    }
  },
  async mounted() {
    await this.$axios
...
<template>
  <VRow>
    ...
    <VCol cols="12">
      <VCard title="Rest Api Permissions">
        <VCardItem>
          <VRow>
            <VCol cols="12" md="6">
              List of all rest api permissions declared in the ESE database.
            </VCol>
            <VCol cols="12" md="6" class="text-right">
              <v-btn variant="outlined" size="small" to="/rest-api-permissions-add">
                New Rest Api permission
              </v-btn>
            </VCol>
          </VRow>
        </VCardItem>
