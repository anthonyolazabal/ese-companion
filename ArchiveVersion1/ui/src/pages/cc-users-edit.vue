    ...
    async update() {
      if (this.item.username && this.item.password) {
        await this.$axios
          ...
          .then((response) => {
            createToast("Control panel user successfully updated", {
              type: 'success',
          ...
          .then(() => {
            ...
            if (this.$route.query.return == "details") {
              router.push('/cc-users-details?id=' + this.$route.query.id);
            } else {
              router.push('/cc-users');
            }
          })
          .catch(() => {
            createToast("Error updating control panel user, contact your admin ! ", {
              type: 'warning',
              position: 'bottom-center',
            })
          });
  ...
  <VRow>
    <VCol cols="12" md="12">
      <v-breadcrumbs>
        ...
        <v-breadcrumbs-item>
          <RouterLink :to="'/cc-users-edit?id' + $route.query.id">
            Edit User
          </RouterLink>
        </v-breadcrumbs-item>
    ...
    <VCol cols="12">
      <VCard title="Edit a control panel user">
        <VCardText>
